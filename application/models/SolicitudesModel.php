<?php
  class SolicitudesModel extends CI_Model{
    ...
    public function crearTutoria($datos) {
      ...
      $ins = $this->db->query("INSERT INTO streaming(idtutor,idmateria,titulo,descripcion,fecha,valor,link) "
    ...
    public function listarTutoriasMateria($idmateria) {
      $mensaje = '';
      $res = $this->db->query("SELECT * FROM streaming WHERE idmateria={$idmateria}");
      if ($res->num_rows()>0) {
        $cont1 = 0;
        foreach($res->result() as $row){
          if ($cont1==0) {
          ...
          $mensaje .= '{"id":"'.($row->id).'","titulo":"'.($row->titulo).'","idtutor":"'.($row->idtutor).'" ,"fecha":"'.($row->fecha).'","descripcion":"'.trim($row->descripcion).'","valor":"'.($row->valor).'"}';
        }
        if ($cont1 > 0) {
          $mensaje = '[' . $mensaje . ']';
        }
